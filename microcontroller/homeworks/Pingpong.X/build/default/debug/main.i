...
void ClearBuffer();
void MyusartRead();
# 5 "./setting_hardaware/setting.h" 2
...
# 6 "./setting_hardaware/setting.h" 2

# 1 "./setting_hardaware/interrupt_manager.h" 1


void INTERRUPT_Initialize(void);
# 7 "./setting_hardaware/setting.h" 2
...
void __attribute__((picinterrupt("high_priority"))) Hi_ISR(void)
{
    ...
    if(PIR1bits.TMR1IF==1){
        ...
        if(ball.x==10 +10&&(player1.y<=ball.y&&player1.y+30>=ball.y)){
            ball.hvelo*=-1;
        }
        if(ball.x==140&&(player2.y<=ball.y&&player2.y+30>=ball.y)){
            ball.hvelo*=-1;
        }

        if(ball.y<=0||ball.y+10>=120) ball.vvelo*=-1;
